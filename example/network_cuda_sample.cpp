...
// Example 2: Training NAND Gate on GPU
void example2_TrainingXOR_GPU() {
    printHeader("EXAMPLE 2: Training 3-Input NAND Gate on GPU");
    
    std::cout << "NAND Gate: Classic logic gate - all inputs high = output low\n\n";
    
    std::cout << BOLD << "3-Input NAND Truth Table:" << RESET << "\n";
    std::cout << "  ┌───┬───┬───┬────────┐\n";
    ...
    // Build network
    std::cout << BOLD << "Building GPU Network: 3 → 4 → 1" << RESET << "\n\n";
    NeuralNetworkCUDA network;
    network.addLayer(new DenseLayerCUDA(3, 4, new ReLUCUDA()));
    network.addLayer(new DenseLayerCUDA(4, 1, new SigmoidCUDA()));
    network.setLoss(new MSELossCUDA());
    network.setOptimizer(new SGD_CUDA(0.1));
    
    // Prepare data (on CPU first, then transfer to GPU)
    // 8 samples for 3-input NAND gate
    Matrix X_cpu(8, 3);
    X_cpu.set(0, 0, 0.0); X_cpu.set(0, 1, 0.0); X_cpu.set(0, 2, 0.0);
    X_cpu.set(1, 0, 0.0); X_cpu.set(1, 1, 0.0); X_cpu.set(1, 2, 1.0);
    X_cpu.set(2, 0, 0.0); X_cpu.set(2, 1, 1.0); X_cpu.set(2, 2, 0.0);
    X_cpu.set(3, 0, 0.0); X_cpu.set(3, 1, 1.0); X_cpu.set(3, 2, 1.0);
    X_cpu.set(4, 0, 1.0); X_cpu.set(4, 1, 0.0); X_cpu.set(4, 2, 0.0);
    X_cpu.set(5, 0, 1.0); X_cpu.set(5, 1, 0.0); X_cpu.set(5, 2, 1.0);
    X_cpu.set(6, 0, 1.0); X_cpu.set(6, 1, 1.0); X_cpu.set(6, 2, 0.0);
    X_cpu.set(7, 0, 1.0); X_cpu.set(7, 1, 1.0); X_cpu.set(7, 2, 1.0);
    
    Matrix y_cpu(8, 1);
    y_cpu.set(0, 0, 1.0);  // 0,0,0 → 1
    y_cpu.set(1, 0, 1.0);  // 0,0,1 → 1
    y_cpu.set(2, 0, 1.0);  // 0,1,0 → 1
    y_cpu.set(3, 0, 1.0);  // 0,1,1 → 1
    y_cpu.set(4, 0, 1.0);  // 1,0,0 → 1
    y_cpu.set(5, 0, 1.0);  // 1,0,1 → 1
    y_cpu.set(6, 0, 1.0);  // 1,1,0 → 1
    y_cpu.set(7, 0, 0.0);  // 1,1,1 → 0 (only case where output is 0)
    ...
    std::cout << "  Input: (8 × 3) matrix\n";
    std::cout << "  Output: (8 × 1) matrix\n\n";
    
    // Train
    std::cout << BOLD << "Training on GPU..." << RESET << "\n";
    network.train(X_train, y_train, 1000, 8, 0.1, true);
    ...
    for (size_t i = 0; i < 8; ++i) {
        std::cout << "[" << X_train.get(i,0) << ", " << X_train.get(i,1) << ", " << X_train.get(i,2) << "]  →  ";
    ...
    std::cout << "\n" << GREEN << "✓ 3-Input NAND Gate Learned on GPU!" << RESET << "\n";
}
